...
<body>
    ...
    // var_dump($_GET['difficulty']);
    // exit;
    ?>
    <main>
        <section class="container">
            <?php
            if (isset($_GET['country']) && isset($_GET['difficulty']) && $_GET['difficulty'] === 'expert') {
                $languageName = $_GET['country'];

                // Prepare the query to fetch beginner courses for the selected language
                $query = $dbCo->prepare("SELECT c.*, l.name AS name FROM course c JOIN languages l ON c.id_language = l.id_language WHERE c.id_difficulty = 3 AND l.country = :languageName");
